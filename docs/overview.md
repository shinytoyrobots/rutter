...
A plain-English overview of the Librarian — what it is, how it works, how to set it up, and where it is heading. It reflects what exists **today**: a first, deliberately small version that searches your vault. The larger vision (a memory that tracks how your thinking changes over time) is designed but not yet built, and is marked clearly as such.

## What the Librarian is

The Librarian is a small program that gives an AI assistant (Claude today, ChatGPT later) fast, reliable access to your Obsidian vault. The long-term goal is a research companion that *remembers across time and thinks alongside you* — the "Librarian" from Neal Stephenson's *Snow Crash*.

Today it does one thing: **it searches your notes and hands back the right ones, each tagged with where and when you wrote it.** That is the foundation the rest is built on.

An important expectation to set early: for plain searching, you already have a good tool — Claude Code reading your organized vault. The Librarian earns its place later, once it can do the thing an ordinary assistant cannot: remember. This first version exists mainly to prove you will actually reach for it.
...
A small always-running process holds that database open and offers two abilities to the AI assistant:

- `librarian-search` — search the vault; return ranked notes with their path, type, status, date, and a matching snippet.
...
Today the Librarian is **reactive**. It does nothing until the assistant calls it. A typical exchange runs like this:
...
3. The Librarian runs the search and returns matching notes with their provenance.
4. Claude reads those results, optionally pulls a full note with `librarian-get-note`, then answers you.

You can also ask directly: "search my vault for X." The Librarian never speaks to you on its own — it answers the assistant, and the assistant answers you.
...
5. Open a new Claude Code session and confirm the connection with the `/mcp` command. You should see `my-librarian` with two tools.
...
Everything above is retrieval. The reason the project exists is the part that comes next — memory an ordinary assistant cannot have:

- **Continuity** — the Librarian remembers what you looked at, so it can answer "what was I working on?" and "have I seen this before?"
...
The full roadmap lives in [`DESIGN.md`](../DESIGN.md). These capabilities also explain a design choice you can see already: the assistant is always the brain, and the Librarian is the memory. Keeping that split is what will let the same Librarian serve both Claude and ChatGPT.
...
- It has no memory yet. That is the next thing to build, and the real test is whether you reach for even this first version.
