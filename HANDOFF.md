...
- **NOT yet done:** dogfooding (the desirability gate hasn't started); S1.5 implementation; anything in H2/H3.
- **Immediate next step:** **`/flow-generate`** for gen-1 (unblocked). Expect ~7 generators at temperature 0.5 per dispatch rules. Branch off `main`; first implementation unknown is the Stop-hook spike (§8, pinned as SCN-001's observable contract).
