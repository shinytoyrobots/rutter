...
- **POST-SHIP FINDING → spec v3.0.0 (HITL-approved):** Stop fires at the end of EVERY assistant turn, not per session; shipped capture has no session dedupe → duplicate entries per turn once a directive exists (cosmetic; hand-deletable). SCN-001 amended to idempotent-per-distinct-directive; SR-013/SR-014 added; suite 0.3.0 (+COR-R-017/018, +COR-A-009). **Shipped var-3 carries a known correctness regression against these until the hotfix.** Spec v3.0.0 is uncommitted.
- **NOT yet done:** (1) **hotfix the capture path** vs SR-013/SR-014 (unblocked — likely `/flow-generate --hotfix` N=1 or a direct narrow fix; touches `capture.ts`/`session-record.ts`); (2) commit/PR spec v3.0.0 + the hotfix together; (3) the 2-week desirability gate — verdict ~2026-08-08 (mandatory HITL); (4) flow-eval cost-budget recalibration (queued); (5) H2/H3 — wish log decides after the gate.
- **Immediate next step:** hotfix SR-013/SR-014, then live with it. Keep the wish log.
