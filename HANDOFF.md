...
- **NOT yet done:** (1) confirm the hotfix PR merged to `main` (in flight at handoff-write time); (2) the 2-week desirability gate — verdict ~2026-08-08 (mandatory HITL: `npm run gate` ≥3×/week unprompted + wish log → H2/H3/stop); (3) flow-eval cost-budget recalibration (every variant to date fails the 150k axis); (4) watch items: O(files) capture scan (~10k files), enrichment O(records)/search; (5) H2/H3 — wish log decides after the gate.
- **Immediate next step:** live with it; keep the wish log; Friday `npm run gate`.
