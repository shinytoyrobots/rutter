...
- **Loose ends at 2026-07-26 session close:** (1) vault has uncommitted `_librarian/` working-tree changes (wish-log SHIPPED annotations + the day's session record) awaiting Robin's manual Obsidian-flow commit/push — do NOT commit them for him (see vault rule below); (2) superseded gen-3 branches (`flow/gen-3/var-2-simplicity`, `var-3-security`) and agent worktrees under `.claude/worktrees/` intentionally preserved; (3) `main` @ PR #6, rebuilt, 69/69 — the provenance-capturing hook is live.
